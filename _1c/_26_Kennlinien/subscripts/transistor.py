...
def plots(T, T_err, __color, I_BE):
    U_CE, I_CE = T
    U_CE_err, I_CE_err = T_err
    plt.errorbar(U_CE, I_CE, xerr=U_CE_err, yerr=I_CE_err, label='$I_{BE} =$' + str(I_BE) + 'mA', color=__color,
                 fmt='o',
                 capsize=5)
    return
...
def signal_boost_worse(I_BE, I_BE_err, I_CE, I_CE_err):
    B = I_CE / I_BE
    B_err = np.sqrt((I_CE_err / I_BE) ** 2 + (I_CE * I_BE_err / I_BE ** 2) ** 2)
    a2t(np.array([I_BE, B]), np.array([I_BE_err, B_err]), [['I', 'B'], ['A','']], 'Die Großsignalverstärkung $B$ In Abhängigkeit des Basisstroms $I_\\mathrm{BE}$.', 'Großsignalverstärkung')
    return
