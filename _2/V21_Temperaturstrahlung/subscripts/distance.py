...
def distance_dependence(distance: np.ndarray, voltage: np.ndarray, ax, messreihe, distance_uncertainty: np.ndarray = np.array([0]),
    ...
    ax.errorbar(x, y, xerr=delta_x, yerr=delta_y, label=f'Messwerte', capsize=5, color=('red' if messreihe == 1 else 'blue'), fmt=('v' if messreihe == 1 else '^'))
    ax.plot(x, model(x, n), label=f'Lineare Regression', color=('green' if messreihe == 1 else 'purple'))
    ax.fill_between(x, model(x, n - delta_n), model(x, n + delta_n), color=('green' if messreihe == 1 else 'purple'), alpha=0.2, label='Unsicherheit')
    ax.set(xlabel='$\\log(r/r_0)$', ylabel='$\\log(U/U_0)$', title=f'Messreihe {messreihe}')
