...
i = np.array([i for i in range(1, 4)])
delta_x_meas = np.array([[0.8, 0.83], [0.82, 0.83], [0.84, 0.86]]) / 1e3
delta_delta_x = np.array([10 for _ in range(len(delta_x_meas))]) / 1e6
N = 200
delta_N = 10
lambda_Laser = 532e-9
...
#a2t(np.array([i, delta_x * 1e3]), np.array([np.zeros_like(i), delta_delta_x * 1e3]), [['i', '$\\Delta x$'],['', 'mm']])

delta_delta_x = delta_delta_x[0] / np.sqrt(len(delta_x))

kappa, delta_kappa = get_kappa(np.mean(delta_x_m), lambda_Laser, N, delta_lambda=1e-9, delta_delta_x=delta_delta_x, delta_N=delta_N)
kappa_r, delta_kappa_r = sci_round(kappa, delta_kappa)

print(kappa, delta_kappa)
print(kappa_r, delta_kappa_r)

delta_x_unknown = np.array([0.96, 0.99, 0.97]) / 1e3
delta_delta_x = np.array([50 for _ in range(len(delta_x_meas))]) / 1e6
delta_x_um = np.mean(delta_x_unknown)
delta_delta_x_u = delta_delta_x[0] / np.sqrt(len(delta_x_unknown))

#a2t(np.array([i, delta_x_unknown * 1e3]), np.array([np.zeros_like(i), delta_delta_x * 1e3]), [['i', '$\\Delta x$'],['', 'mm']])

lambda_unknown, delta_lambda = get_lambda(kappa, np.mean(delta_x_unknown), N, delta_kappa=delta_kappa, delta_delta_x=delta_delta_x_u, delta_N=delta_N)
lambda_r, delta_lambda_r = sci_round(lambda_unknown, delta_lambda)

print(lambda_unknown, delta_lambda)
